...
class ApiService {
  ...
  Future<List<Movie>> getMovies(String type, {int page = 1}) async {
    ...
    final url = Uri.parse(
      '$_baseUrl$endpoint${separator}api_key=$_apiKey&language=vi-VN&page=$page',
    );
  ...
  Future<String?> getMovieStreamLink(
    ...
    try {
      final tmdbUrl = Uri.parse('https://phimapi.com/tmdb/movie/$tmdbId');
      final tmdbRes = await http.get(tmdbUrl);

      if (tmdbRes.statusCode == 200) {
        final data = json.decode(tmdbRes.body);
        if (data['episodes'] != null && data['episodes'].isNotEmpty) {
          return data['episodes'][0]['server_data'][0]['link_m3u8'];
        }
      }
    } catch (e) {
      print("Lỗi API TMDB ID (Có thể Server chưa map ID này): $e");
    }

    print("Chuyển sang quét dự phòng bằng tên phim...");
